...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const mess = {
            wait: '[ WAIT ] Sedang di proses Sayang⏳ silahkan tunggu sebentar Sayang',
            error: {
        ...
        const ownerNumber = '6282189403925@c.us'
        ...
        const barbarkey = '1GJyt4zA55Q1TDx9z9cA' // https://mhankbarbar.herokuapp.com/api
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh bapak Kapten!', id)
                        ...
                        if(quotedMsgObj == null)
                        {
                            ...
                            if(gasMake.status == true)
                            {
                                try{
                                    ...
                                    await tobz.reply(from, 'Gagal membuat sayang.', id)
                                } 
                            }else{
        ...
        case '#sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    tobz.reply(from, '[WAIT] Sedang di proses Sayang⏳ silahkan tunggu ± 1 min sayang!', id)
        ...
        case '#stickertoimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                tobz.reply(from, '[WAIT] Sedang di proses Sayang⏳ silahkan tunggu sayang!', id)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
...
${desc}`)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `6282189403925'@c.us`)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Wildan!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
